package es.dtr.job.interview.commons.data.querydsl;
...
public interface QueryDslRepository<T, K extends EntityPathBase<T>>
    ...
    default List<T> findAll(final Class<T> entityType, final List<QueryDslFilter> filters) {
        return findAll(entityType, filters, null);
    }
    ...
    default List<T> findAll(final Class<T> entityType, final List<QueryDslFilter> filters,
        ...
        if (CollectionUtils.isNotEmpty(filters)) {
            final QueryDslPredicateBuilder<T> queryDslPredicateBuilder = new QueryDslPredicateBuilder<T>(entityType)
                    .with(filters);
